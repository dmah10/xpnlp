...
class BiGRULanguageModel(nn.Module):
    def __init__(self, config, vocab, pad_token=0, device=0):
        ...
        self.encoder = DynamicEncoder(
            self.n_vocab + 2, self.embed_size, self.hidden_size, self.gpu
        )
    ...
    def append_start_end_tokens(self, inp, inp_length):
        ...
        start_tokens = (
            torch.LongTensor([self.fw_start_token] * batch_size)
            .view(1, -1)
            .to(inp.device)
        )  # [1,B]
        end_tokens_pad = (
            torch.LongTensor([self.pad_token] * batch_size).view(1, -1).to(inp.device)
        )  # [1,B]
    ...
    def sample_n(self, method, batch, max_sample_length, sample_num):
        ...
        fw_hidden = torch.cat(
            [fw_hidden[:, :, : self.hidden_size], fw_hidden[:, :, self.hidden_size :]],
            0,
        )
        ...
        bw_hidden = torch.cat(
            [bw_hidden[:, :, : self.hidden_size], bw_hidden[:, :, self.hidden_size :]],
            0,
        )
        bw_next_token = padded_inp.index_select(0, batch.bw_pos + 2).view(1, -1)

        fw_sample_outputs = self.sample_n_sequences(
            method, "fw", fw_next_token, fw_hidden, max_sample_length, sample_num
        )
        bw_sample_outputs = self.sample_n_sequences(
            method, "bw", bw_next_token, bw_hidden, max_sample_length, sample_num
        )
    ...
    def filter_special_tokens(self, m):
        for i in range(m.size(0)):
            for j in range(m.size(1)):
                if (
                    m[i, j] >= self.n_vocab - 2
                    or m[i, j] == self.vocab.get("[CLS]", 0)
