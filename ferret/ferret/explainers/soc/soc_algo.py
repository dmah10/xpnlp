...
class _SamplingAndOcclusionBaseAlgo:
    ...
    def get_ngram_mask_region(self, region, inp):
        ...
        while idx < len(inp) and inp[idx] != 0:
            idx += 1
        return [
            (max(region[0] - self.nb_range, 1), min(region[1] + self.nb_range, idx - 2))
        ]
...
class _SamplingAndOcclusionAlgo(_SamplingAndOcclusionBaseAlgo):
    ...
    def occlude_input_with_masks_and_run(
        self,
        inp_bak,
        inp_mask,
        segment_ids,
        ...
        for sample_i in range(self.sample_num + extra):
            ...
            for i in range(len(filled_inp)):
                if self.configs.keep_other_nw or not (
                    i in additional_mask and not x_region[0] <= i <= x_region[1]
                ):
            ...
            for i in range(len(filled_inp)):
                if not x_region[0] <= i <= x_region[1] and (
                    i not in additional_mask or self.configs.keep_other_nw
                ):
        ...
        inp_enb_mask, inp_ex_mask = (
            torch.from_numpy(inp_enb_mask).long(),
            torch.from_numpy(inp_ex_mask).long(),
        )

        if self.gpu >= 0:
            inp_enb, inp_ex = inp_enb.to(self.gpu), inp_ex.to(self.gpu)
            inp_enb_mask, inp_ex_mask = inp_enb_mask.to(self.gpu), inp_ex_mask.to(
                self.gpu
            )
    ...
    def do_attribution(
        ...
        score = self.occlude_input_with_masks_and_run(
            inp_flatten,
            inp_mask_flatten,
            segment_ids,
            [region],
    ...
    def do_attribution_pad_variant(
        self,
        input_ids,
        input_mask,
        segment_ids,
        ...
        inp_enb, inp_mask_enb, inp_ex, inp_mask_ex = self.occlude_input_with_masks(
            inp_flatten, inp_mask_flatten, [region], mask_regions, additional_mask
        )
    ...
    def do_hierarchical_explanation(
        ...
        lists = self.agglomerate(
            ...
            method="cd",
            sweep_dim=1,
            num_iters=self.hiex_tree_height,
